...
describe('CardEffect', () => {
  ...
  describe('#apply()', () => {
    it('should add health to the legend card for ADD_HEALTH', () => {
      ...
      const legendCardMock = mock(LegendCard);
      const currentPlayerMock = mock(Player);

      // when(legendCardMock.currentHealth).thenReturn(20);
      legendCardMock.currentHealth = 20;

      const legendCard = instance(legendCardMock);

      when(currentPlayerMock.legendCard).thenReturn(legendCard);

      const currentPlayer = instance(currentPlayerMock);

      when(stateMock.currentPlayer).thenReturn(currentPlayer);

      const state = instance(stateMock);

      console.log(state.currentPlayer!.legendCard.currentHealth);
