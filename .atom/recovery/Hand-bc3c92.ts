...
export class Hand extends PIXI.Container {
  ...
  resize(screenWidth: number, screenHeight: number): void {
    ...
    if (sprites.length > 0) {
      const margin = screenWidth / 50;
      const availableWidth = screenWidth - (sprites.length - 1) * margin;
      const availableHeight = screenHeight;

      sprites.forEach(sprite => (sprite.scale.x = sprite.scale.y = 1));

      const currentWidth = sprites
        .map(child => child.width)
        .reduce((totalWidth, width) => (totalWidth += width));
      const currentScale = availableWidth / currentWidth;
